...
def gaussian_kernel(sigma):
    """Create a 2D Gaussian kernel with the specified standard deviation."""
    size = int(2 * np.ceil(2 * sigma) + 1)
    x, y = np.mgrid[-size//2 + 1:size//2 + 1, -size//2 + 1:size//2 + 1]
    g = np.exp(-((x**2 + y**2)/(2.0*sigma**2)))
    return g / np.sum(g)

def downsample_gaussian(image, factor, sigma):
    """Downsample an image by a factor using Gaussian smoothing."""
